...
namespace Matriz
{
    public unsafe class MatrizDispersa<T> where T : unmanaged
    {
        ...
        public void insert(int idVehiculo, int idRepuesto, Servicio servicio)
        {
            ...
            nuevo->id = servicio.ID; // Asigna el ID del servicio al nodo
            nuevo->nombre = servicio.Detalles; // Asigna los detalles del servicio al nodo
            nuevo->coordenadaX = idVehiculo; // Asigna la coordenada X (vehículo)
            nuevo->coordenadaY = idRepuesto; // Asigna la coordenada Y (repuesto)
            nuevo->arriba = null;
            nuevo->abajo = null;
            nuevo->derecha = null;
            nuevo->izquierda = null;

            // Verificar si ya existen los encabezados para la fila (vehículo) y columna (repuesto) en la matriz
            NodoEncabezado<int>* nodo_X = filas.getEncabezado(idVehiculo); // Obtener el encabezado de la fila
            NodoEncabezado<int>* nodo_Y = columnas.getEncabezado(idRepuesto); // Obtener el encabezado de la columna

            // Si el encabezado de la fila no existe, crearlo
            if (nodo_X == null)
            {
                filas.insertar_nodoEncabezado(idVehiculo); // Crear encabezado para la fila
                nodo_X = filas.getEncabezado(idVehiculo); // Obtener el encabezado de la fila recién creado
            }

            // Si el encabezado de la columna no existe, crearlo
            if (nodo_Y == null)
            {
                columnas.insertar_nodoEncabezado(idRepuesto); // Crear encabezado para la columna
                nodo_Y = columnas.getEncabezado(idRepuesto); // Obtener el encabezado de la columna recién creado
            }
            ...
            else
            {
                ...
                while (tmp != null)
                {
                    ...
                    if (nuevo->coordenadaY < tmp->coordenadaY)
                    {
                        ...
                        if (tmp->izquierda != null) tmp->izquierda->derecha = nuevo;
                        tmp->izquierda = nuevo;
                        if (tmp == nodo_X->acceso) nodo_X->acceso = nuevo;
                        break;
                    }
            ...
            else
            {
                ...
                while (tmp2 != null)
                {
                    ...
                    if (nuevo->coordenadaX < tmp2->coordenadaX)
                    {
                        ...
                        if (tmp2->arriba != null) tmp2->arriba->abajo = nuevo;
                        tmp2->arriba = nuevo;
                        if (tmp2 == nodo_Y->acceso) nodo_Y->acceso = nuevo;
                        break;
                    }
        ...
        public void mostrar()
        {
            ...
            {
                Console.Write("R" + y_columna->id + "\t"); // Imprimir el encabezado de cada columna
        ...
        // Método para generar el código Graphviz de la matriz dispersa
        public string GenerarGraphviz()
        {
            // Si la matriz está vacía, generamos un solo nodo con "NULL"
            ...
            graphviz += "        label = \"Matriz Dispersa\";\n";

            // Crear nodos para los encabezados de las columnas
            NodoEncabezado<int>* y_columna = columnas.primero;
            while (y_columna != null)
            {
                graphviz += $"        c{y_columna->id} [label = \"R{y_columna->id}\"];\n";
                y_columna = y_columna->siguiente;
            }

            // Crear nodos para los encabezados de las filas y sus valores
            NodoEncabezado<int>* x_fila = filas.primero;
            while (x_fila != null)
            {
                graphviz += $"        f{x_fila->id} [label = \"V{x_fila->id}\"];\n";
                NodoInterno<int>* interno = x_fila->acceso;
                while (interno != null)
                {
                    graphviz += $"        n{interno->coordenadaX}_{interno->coordenadaY} [label = \"{interno->nombre}\"];\n";
                    interno = interno->derecha;
                }
                x_fila = x_fila->siguiente;
            }

            // Conectar los nodos de los encabezados de las filas y columnas
            x_fila = filas.primero;
            while (x_fila != null)
            {
                graphviz += $"        f{x_fila->id} -> n{x_fila->id}_1;\n";
                NodoInterno<int>* interno = x_fila->acceso;
                while (interno != null)
                {
                    if (interno->derecha != null)
                    {
                        graphviz += $"        n{interno->coordenadaX}_{interno->coordenadaY} -> n{interno->coordenadaX}_{interno->derecha->coordenadaY};\n";
                    }
                    if (interno->abajo != null)
                    {
                        graphviz += $"        n{interno->coordenadaX}_{interno->coordenadaY} -> n{interno->abajo->coordenadaX}_{interno->coordenadaY};\n";
                    }
                    interno = interno->derecha;
                }
                x_fila = x_fila->siguiente;
            ...
            // Conectar los nodos de los encabezados de las columnas
            y_columna = columnas.primero;
            while (y_columna != null)
            {
                graphviz += $"        c{y_columna->id} -> n1_{y_columna->id};\n";
                y_columna = y_columna->siguiente;
            }
